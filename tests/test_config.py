...
from use_nacos.client import NacosAsyncClient, NacosClient
from use_nacos.endpoints import ConfigAsyncEndpoint, ConfigEndpoint
from use_nacos.endpoints import config as conf
...
def test_config_get_not_found_default_value(config, mocker):
    mocker.patch.object(
        ConfigEndpoint,
        "_get",
        side_effect=HTTPResponseError(response=httpx.Response(404)),
    )
    assert (
        config.get("test_config_miss", "DEFAULT_GROUP", default="default_value")
        == "default_value"
    )
...
@pytest.mark.parametrize(
    "content ,tenant, type, serialized, expected",
    [
...
def test_config_publish_get(
    config, data_id, group, content, tenant, type, serialized, expected
):
    assert config.publish(data_id, group, content, tenant, type)
    assert config.get(data_id, group, tenant, serializer=serialized) == expected
...
def test_config_subscriber(config):
    ...
    config_subscriber = config.subscribe(
        data_id=dataId, group="DEFAULT_GROUP", callback=_callback
    )
    # update config
...
@pytest.mark.parametrize(
    "content ,tenant, type, serializer, expected",
    [
...
@pytest.mark.asyncio
async def test_async_config_publish_get(
    async_config, data_id, group, content, tenant, type, serializer, expected
):
    assert await async_config.publish(data_id, group, content, tenant, type)
    assert (
        await async_config.get(data_id, group, tenant, serializer=serializer)
        == expected
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "DEFAULT_GROUP",
            "test_tenant",
            "test_config#DEFAULT_GROUP#test_tenant",
        ),
    ],
)
...
def test_mock_exception(config, mocker):
    mocker.patch.object(
        ConfigEndpoint,
        "_get",
        side_effect=HTTPResponseError(response=httpx.Response(500)),
    )
    with pytest.raises(HTTPResponseError):
